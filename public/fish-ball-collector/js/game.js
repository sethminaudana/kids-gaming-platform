...
function gameOver() {
    ...
    if (gs.finalScoreElement) {
        if (gs.currentLanguage === 'sinhala') {
            gs.finalScoreElement.textContent = `අවසාන ලකුණු: ${gs.score}`;
        } else if (gs.currentLanguage === 'tamil') {
            gs.finalScoreElement.textContent = `இறுதிப் புள்ளிகள்: ${gs.score}`;
        } else {
            gs.finalScoreElement.textContent = `Final Score: ${gs.score}`;
        }
    }
...
function initGame() {
    ...
    document.getElementById('voiceOnBtn').addEventListener('click', () => {
        ...
        if (gs.speechSynthesisSupported && window.speakText) {
            ...
            if (gs.currentLanguage === 'sinhala') testMessage = "හලෝ!";
            if (gs.currentLanguage === 'tamil') testMessage = "ஹலோ!";
    ...
    document.getElementById('tamilBtn').addEventListener('click', () => {
        const gs = window.gameState;
        if (gs.currentLanguage !== 'tamil') {
            gs.currentLanguage = 'tamil';
            document.getElementById('tamilBtn').classList.add('active');
            document.getElementById('englishBtn').classList.remove('active');
            document.getElementById('sinhalaBtn').classList.remove('active');

            if (gs.canvas && window.showMessage) {
                window.showMessage("தமிழ் மொழி", "bad", gs.canvas.width / 2, gs.canvas.height / 2);
            }
