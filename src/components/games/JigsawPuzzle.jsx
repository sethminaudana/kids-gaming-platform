...
const JigsawPuzzle = () => {
  ...
  const loadChildren = async () => {
    try {
      ...
      showFeedback('info', 'Please add a child profile to save progress');
    } finally {
  ...
  const saveToBackend = async (timeTaken, mouseMetrics) => {
    if (!selectedChild) {
      console.log('⚠️ No child selected, cannot save to backend');
      return;
    }
    ...
    try {
      const gameData = {
        ...
        mouseData: {
          mousePath: mousePath.slice(-100) // Send last 100 points
        }
      };
      
      console.log('📤 Sending game data to backend:', gameData);
      const response = await apiService.saveGameSession(gameData);
      console.log('✅ Game session saved to backend successfully:', response);
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${puzzleImages[difficulty].colors} p-4 md:p-6`}>
      <div className="max-w-7xl mx-auto">
        ...
        {children.length > 0 && (
          <div className="mb-6 bg-white bg-opacity-80 backdrop-blur-sm rounded-2xl p-4 shadow-lg">
            <div className="flex flex-wrap items-center justify-between">
              <label className="text-gray-700 font-medium">Select Child:</label>
              <select
                value={selectedChild || ''}
              ...
              >
                {children.map(child => (
                  <option key={child._id} value={child._id}>
                    {child.childName} ({child.childAge} years)
                  </option>
                ))}
              </select>
            </div>
        ...
        {!loadingChildren && children.length === 0 && (
          <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-2xl p-6 text-center">
            <p className="text-yellow-800 text-lg mb-3">
              👋 Please add a child profile to save your game progress!
            </p>
            <button
              onClick={goToAddChild}
              className="px-6 py-3 bg-gradient-to-r from-yellow-500 to-orange-500 text-white rounded-lg hover:from-yellow-600 hover:to-orange-600 font-bold"
            >
              Add Child Profile
            </button>
          </div>
